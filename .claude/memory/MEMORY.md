...
- [Lambda Log Group Verify](feedback-lambda-log-group-verify.md) — never assume a Lambda's CloudWatch log group is the AWS default; check its actual LoggingConfig first (template.yaml sets a custom shared group)
- [Realtime Secret Fetch Standard](project-realtime-secret-fetch-standard.md) — all secret fetches must use getSecret()+cache+auth-retry, not CFN-baked env vars; VPC-attached Lambdas with no Secrets Manager route are the sole exception
- [HSHD Data Classification](feedback-hshd-data-classification.md) — HSHD (PAN/SSN/Passport etc.) must be encrypted at rest with decrypt-access tracked; plain PII may be plaintext but access still tracked
- [Idempotent Writes](feedback-idempotent-writes.md) — all data-update endpoints must be idempotent (user-mandated, permanent); safety precondition for the DB resilience strategy's "retry the write" advice
...
- [Billing Metrics Collector](project-billing-metrics-collector.md) — jobs/collect-metrics.js rewritten to call the tenant's own GET /api/billing/metrics via billing-tenant-client.js instead of a direct Postgres connection; old version was broken 3 ways + never had real prod credentials
- [Billing Commission Computation](project-billing-commission-computation.md) — GET /api/invoices/suggest-line-items actually applies sales_pct/rental_pct to billing_metrics now; "Rental %"/"Sales %" used to be dropdown labels only, staff computed the amount by hand
