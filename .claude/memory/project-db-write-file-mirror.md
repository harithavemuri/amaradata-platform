...
**On-demand full sync (site_admin UI):** `db.js` exports `mirrorTableToFile(table)` (resolves with the row count written) specifically so `POST /api/admin/sync-from-db` (admin.js route, `requireSiteAdmin`) can loop every `metadata/manifest.json` table and re-export all of them at once, rather than waiting for the next incidental write to each. Button: **"⇅ Sync from DB"** on `frontend/admin-health.html` (hidden in NonDB mode).
