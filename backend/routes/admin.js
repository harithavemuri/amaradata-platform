...
// One-off cleanup route: no invoices have actually been raised for Rohas yet,
// so the 5 seed/test invoices currently in prod need to be cleared before
// real invoicing starts. Use once against production, then remove this route
// and redeploy — same pattern as the earlier purge-test-tenants cleanup.
router.post('/purge-invoices', async (req, res) => {
    try {
        if (req.db.mode === 'nondb') {
