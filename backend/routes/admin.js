...
// POST /api/admin/purge-test-tenants
//
// One-off cleanup route: deletes every tenant except keep_tenant_id, plus every
// row in tenant-scoped tables that references a deleted tenant. Added to clean
// up 231 tenants worth of accumulated Playwright/regression test pollution
// found in production (tenant names like "Enh Tenant", "Acme Corp", "Named
// Tenant <random>" — exactly what the test suites generate). Meant to be
// removed again once used; not a permanent feature.
//
// group_tenant and invoice_line_items cascade automatically (ON DELETE CASCADE
// in schema.sql); every other tenant_id-referencing table does not, so they're
// deleted explicitly in dependency order, in a single transaction:
// billing_metrics (-> tenant_subscriptions) and payments/enhancements
// (-> invoices) must go before the tables they reference.
router.post('/purge-test-tenants', async (req, res) => {
    const { keep_tenant_id } = req.body;
    if (!keep_tenant_id) return res.status(400).json({ error: 'keep_tenant_id required' });

    try {
        if (req.db.mode === 'nondb') {
            const fileDb = req.db.fileDb;
            const results = {};

            for (const table of ['billing_metrics', 'payments', 'enhancements', 'tenant_subscriptions']) {
                const toDelete = fileDb.find(table).filter(r => r.tenant_id != keep_tenant_id);
                toDelete.forEach(r => fileDb.delete(table, r.id));
                results[table] = toDelete.length;
            }

            const invoicesToDelete = fileDb.find('invoices').filter(r => r.tenant_id != keep_tenant_id);
            invoicesToDelete.forEach(inv => {
                fileDb.find('invoice_line_items')
                    .filter(li => li.invoice_id == inv.id)
                    .forEach(li => fileDb.delete('invoice_line_items', li.id));
                fileDb.delete('invoices', inv.id);
            });
            results.invoices = invoicesToDelete.length;

            const tenantsToDelete = fileDb.find('tenants').filter(t => t.id != keep_tenant_id);
            tenantsToDelete.forEach(t => {
                fileDb.find('group_tenant')
                    .filter(gt => gt.tenant_id == t.id)
                    .forEach(gt => fileDb.delete('group_tenant', gt.id));
                fileDb.delete('tenants', t.id);
            });
            results.tenants = tenantsToDelete.length;

            return res.json({ success: true, data: results });
        }

        const client = await db.writePool.connect();
        try {
            await client.query('BEGIN');
            const results = {};
            let r;
            r = await client.query('DELETE FROM billing_metrics WHERE tenant_id <> $1',        [keep_tenant_id]); results.billing_metrics       = r.rowCount;
            r = await client.query('DELETE FROM payments WHERE tenant_id <> $1',              [keep_tenant_id]); results.payments              = r.rowCount;
            r = await client.query('DELETE FROM enhancements WHERE tenant_id <> $1',           [keep_tenant_id]); results.enhancements          = r.rowCount;
            r = await client.query('DELETE FROM tenant_subscriptions WHERE tenant_id <> $1',   [keep_tenant_id]); results.tenant_subscriptions  = r.rowCount;
            r = await client.query('DELETE FROM invoices WHERE tenant_id <> $1',               [keep_tenant_id]); results.invoices              = r.rowCount;
            r = await client.query('DELETE FROM tenants WHERE id <> $1',                       [keep_tenant_id]); results.tenants               = r.rowCount;
