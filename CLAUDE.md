...
| `/api/email` | `backend/routes/email.js` | `requireAuth` + `requireAdmin` per-route (inbox/send/reply via SES + S3) |
...
**Email page testing:** `src/test/email-routes.test.js` covers `/api/email/*` (auth guards, inbox parsing/sort order, attachment download, 400/404/500 paths, send/reply) with the S3 client stubbed as above — SES calls are never mocked since dev-mode already no-ops them. `testing/regression_testsuite/email.spec.js` covers the UI: page load, the real EMAIL_BUCKET-not-configured error state (accurate for this suite's server, which never sets it), compose modal open/cancel, client-visible validation-failure alert, and a full compose→send→modal-closes happy path (safe because of `sendRaw()`'s dev-mode guard). Reply and inbox-with-real-data aren't covered at the E2E layer — that needs `EMAIL_BUCKET` pointed at a real bucket, which no test tier in this repo sets up.
